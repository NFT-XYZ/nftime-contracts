...
contract NFTIME is Ownable, AccessControl, ERC721URIStorage, ERC721Enumerable, ERC721Pausable, ERC721Burnable {
    ...
    DateTime private dateTimeUtil = new DateTime();
    ...
    function mint(uint256 _time) public payable whenNotPaused returns (uint256) {
    ...
    function getAttributes(Date memory date, bool isRarity) internal pure returns (string memory) {
    ...
    function concatAttribute(string memory label, string memory attribute) internal pure returns (string memory) {
        return string.concat("{", '"trait_type": "', label, '", "value": "', attribute, '"}');
    }
    ...
    function setRenderer(Renderer _renderer) external onlyRole(DEFAULT_ADMIN_ROLE) {
        renderer = _renderer;
    }
