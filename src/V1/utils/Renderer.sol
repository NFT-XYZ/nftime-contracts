...
contract Renderer {
    function render(Date memory date) public pure returns (string memory) {
        return string.concat(prepareSVGStyle(), renderDayAttributes(date), renderClockAttributes(date));
    }
    ...
    function renderDayAttributes(Date memory date) internal pure returns (string memory) {
        return string.concat(
            ...
            getDayAttribute(date.dayOfWeek, "138", "200", "440", "520", "760"),
            getDayAttribute(Strings.toString(date.year), "142", "200", "440", "520", "40"),
    ...
    function renderClockAttributes(Date memory date) internal pure returns (string memory) {
        return string.concat(
            getSVGPath("M520 620h200m40 0h200M520 860h440M520 380h440M520 140h440M40 260h440", "black"),
            getSVGPath(
            ...
            getSVGPath(
                "M350.5 745.9c0 2.2-1.8 4-4 4h-12c-2.2 0-4-1.8-4-4v-12c0-2.2 1.8-4 4-4h12c2.2 0 4 1.8 4 4v12z", "white"
            ),
            "</svg>"
        );
    }
    ...
    function getSVGPath(string memory d, string memory stroke) internal pure returns (string memory) {
    ...
    {
        string memory style =
            string.concat("transform: translate(119px, -3.2px) rotate(", computeRotation(rotation, product), "deg);");

        return svg.path(
            string.concat(
                svg.prop("stroke", "white"), svg.prop("stroke-width", "8"), svg.prop("style", style), svg.prop("d", d)
            ),
            ""
        );
    }

    function computeRotation(uint256 rotation, uint256 product) internal pure returns (string memory) {
